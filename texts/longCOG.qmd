...
# clear environment
rm( list = ls()[ !ls() %in% c("d","df","scl","tests","doms","v","rprint","mread","nms","imp","cbPal") ] )
...
rhatmax <- sapply( names(m) , function(i) max(m[[i]]$rhats, na.rm = T ) ) %>% max()
...
Both predictive longitudinal GLMMs converged within a specified number of iterations ($\hat{R}$ 2) with all observations having Pareto-k below 0.7.
