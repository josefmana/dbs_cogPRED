...
# clear environment
rm( list = ls()[ !ls() %in% c("v","rprint","nms") ] )
...
m <-
  ...
  lapply(
    # read descriptive linear and non-linear (T2 splines) models
    setNames( c("m0_linear","m0_spline"), c("Linear","Non-linear") ),
    function(i)
...
dia <- 
  ...
  cbind.data.frame(
    Rhat = round( sapply( names(m) , function(i) max( rhat(m[[i]]), na.rm = T ) ), 3 ),
    Park = round( sapply( names(m) , function(i) max( loo(m[[i]])$diagnostics$pareto_k, na.rm = T ) ), 3 )
  )

# diagnostics for in-text reporting
rhatmax <- with( dia, ifelse( round(max(Rhat),2) > max(Rhat), rprint(max(Rhat),2), rprint(max(Rhat)+.01,2) ) )
parkmax <- with( dia, ifelse( round(max(Park),1) > max(Park), rprint(max(Park),1), rprint(max(Park)+.1,1) ) )
