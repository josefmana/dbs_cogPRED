...
contr <-
  ...
  mutate(
    Contrast =
      case_when(
        Contrast == "Y-0.3" ~ "Pre-surgery",
        grepl( "Y0.7", Contrast ) ~ "Yearly decline",
        .default = Contrast
      )
  )
...
t4 <-
  ...
  mutate( block = ifelse( grepl("minus",Contrast), "Contrasts", "Linear model parameters" ) )
...
t4 <-
  ...
  tab_footnote(
        locations = cells_row_groups( groups = "Linear model parameters" ),
        footnote = "The rows represents expactation of patients' performance at pre-surgery assessment, i.e., 0.3 years before surgery, and expected annual Dementia Rating Scale decline."
    ) %>%
  tab_footnote(
...
# clear environment
rm( list = ls()[ !ls() %in% c("d","df","scl","tests","doms","v","itstats","rprint","ubound","mread","nms","imp","cbPal","b","contr") ] )
...
rm( list = ls()[ !ls() %in% c("itstats","rprint","contr") ] ) # clear environment
...
## Expected post-surgery cognitive decline
...
prev <-
  ...
  # extract data from Smeding et al. (2011) and Gruber et al. (2019)
  # Smeding et al. (2011): Table 2 (mean change score and its SD) and N from "Results: Group characteristics"
  ...
  `rownames<-`( c("smeding2011","gruber2019","reich2022") ) %>%
