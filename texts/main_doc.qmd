...
ggsave(
  plot = ( f4$`test scores` | f4$`factor scores` ) +
    plot_annotation( tag_levels = "A" ) &
    theme( plot.tag = element_text(face = "bold") ),
...
![Interaction terms of the “test scores” (A) and the “factor scores” (B) models predicting post-surgery cognitive decline. Acronyms are explained in the text.](../figs/posterior_summaries.jpg){#fig-postsum width}
...
s <- readRDS( here("mods","sims.rds") ) # read the simulations
...
falsepos <-
  ...
  # extract numbers of simulaions with at least one false positive
  filter( meth == "t0" & fp != 0 ) %>%
