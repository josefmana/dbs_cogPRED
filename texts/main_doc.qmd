...
$$P(DRS_i = DRS_{max}) = 1 - T(\vartheta,\mu_i,\sigma), for DRS_i \in N_{max}, N_{max} = \{i: drs_i = drs_{max}\}$$ $$DRS_i \sim~ t(\vartheta,\mu_i,\sigma), for DRS_i \in N_1, N_1 = \{i: drs_i < drs_{max}\}$$ $$\mu_i = \alpha + \delta_{time}time_i + \sum_{j=1}^{m} (\beta_{predictor[j]}predictor_{[j]i} + \delta_{predictor[j]}time_ipredictor_{[j]i}) + \tau_{\bar{\alpha}}{z}_{\bar{\alpha},id[i]} + \tau_{\bar{\delta}}{z}_{\bar{\delta},id[i]}time_i$$

*i* = 1…*n*, where *n* is the total number of assessments across all patients, *m* is the total number of pre-surgery predictors, $DRS_{max}$ is the maximal attainable score in DRS-2 (i.e., a raw score of 144), *T()* is the Student-t cumulative distribution function, *t()* is the Student-t probability density function, $time_i$ is the time from surgery at assessment *i*, $predictor_{[j]i}$ is the pre-surgery cognitive score in the predictor (i.e., either a test or latent factor) *j* of the patient evaluated at assessment *i*, **$\tau$ parameters represent patient-level variance, $z_{id}$ represent standardized patient-level effects, $\mu_i$ represents true score estimate of patient's** ***id[i]*** **cognitive performance on assessment at $time_i$**, and remaining terms denote model parameters. E**mpirical e**stimands relating to *RQ2* comprised of the two sets of $\delta_{predictor[j]}$ representing the expected prognostic value of single pre-surgery cognitive tests and latent cognitive factors.
...
f5 <-
  ...
  # prepare it
  contr %>%
  filter( grepl( "minus", Contrast ) ) %>%
  mutate(
    `Generated by: ` =
      factor(
        `Generated by: `,
        levels = c(paste0( "Group-",c("level parameters"," & Patient-level parameters")),"Full Model"),
        ordered = T
      )
  ) %>%
    ...
    # plot it
    ggplot() +
    aes( y = Contrast, x = md, xmin = eti_low, xmax = eti_hig, colour = `Generated by: ` ) +
    geom_pointrange( linewidth = 5, position = position_dodge(width = .3), alpha = .4 ) +
    geom_point( size = 5, position = position_dodge(width = .3) ) +
    geom_vline( xintercept = 0, linewidth = 1, linetype = "dashed", colour = "navyblue" ) +
    labs( y = NULL, x = parse( text = "Delta~`DRS-2`" ) ) +
    ...
    scale_colour_manual( values = cbPal[c(8,7,1)] )
...
![Interaction terms of the “test scores” (left column) and the “factor scores” (right column) models predicting post-surgery cognitive decline. Acronyms are explained in the text.](../figs/posterior_distributions.jpg){#fig-postdist}
...
In the present study, we analyzed retrospectively sampled data of longitudinally followed `r itstats[['n']][['incl']]` PD patients after STN DBS surgery and described their post-surgery cognitive performance. We observed a mild **average** post-surgery cognitive decline **with considerable inter-individual variability** (*RQ1*) that was faster in patients with lower pre-surgery executive functions/attention (*RQ2*) compared to the rest of the sample. **Our exploratory analysis of the association between affected proportion of STN components and post-surgery cognitive decline did not yield any clear results, however some patterns of our results can be used to guide further research.
...
**E**xpected average annual rate of cognitive decline after STN DBS in **our sample of** PD **patients** reache**d** 0.90 from a total of 144 points **in DRS-2. Whereas the sample estimate was tightly compatible with this circa one point a year post-surgery cognitive decline, generalization to a population of patients selected via the CAPSIT protocol criteria resulted in estimates of annual post-surgery change ranging from 2.5 points deficit to 0.5 points improvement with 90% certainty (@tbl-contr). For European Americans, @pedraza2007 reported values of 6 and 7 DRS-2 points/year decline as reliable change cutoffs based on 90% predictive intervals for approximately one- and two-years re-tests respectively. As both our sample and population estimates fall safely above these cut-offs, we may conclude that STN DBS seems to be safety from cognitive point of view at least up to two years post-surgery.^[Based on the descriptive linear model reported in @tbl-contr, posterior probability of seeing patient with true score change less than these reliable change cut-offs was respectively `r rci[1,2]`, `r rci[2,2]` and `r rci[3,2]` for the sample true score, population true score and observed score prediction one year after surgery, and `r rci[1,3]`, `r rci[2,3]` and `r rci[3,3]` for the sample true score, population true score and observed score prediction two years after surgery.]**
...
**The estimate of an average annual cognitive decline from our study** represents **somewhat** slower rate **of decline** than in previous reports. **For example, @gruber2019 reported yearly `r prev['gruber2019','abs_diff']` DRS-2 points/year change scores decline (90% CI `r prev['gruber2019','CI']`) in `r prev['gruber2019','n']` patients assessed pre-surgery and at various points post-surgery (compare to the "Yearly decline" row of @tbl-contr). Furthermore, @smeding2009 and @reich2022 calculated one year post-surgery change scores of `r prev['smeding2009','n']` and `r prev['reich2022','n']` patients and reported declines of `r prev['smeding2009','abs_diff']` (90% CI `r prev['smeding2009','CI']`) and `r prev['reich2022','abs_diff']` (90% CI `r prev['reich2022','CI']` DRS-2 points respectively (compare to the "Y1-minus-Pre" row of @tbl-contr). To demonstrate how our linear descriptive model can be used to inform decisions in future studies, we next compare its predictions directly to data reported by @reich2022. We have selected this article because it represents a prototypical pre-surgery/post-surgery study design used to assess cognition in PD patients treated by STN-DBS, it uses DRS-2 as its outcome, and the raw data (change scores) used in the article can be readily identified from the Figure 4B therein.**
...
**To decide whether data such as those evaluated by @reich2022 constitute true score changes (as is assumed by change scores analysis) or whether they could be tainted by appreciable amount of measurement error, we can compare the data distribution with the population true score estimate of our model (@tbl-contr, second column). Our model implies that not more than 5% of the sample should fall below `r lbound` year one post-surgery deficit in the true DRS-2 score. Inspected data set contains `r bottom5percent["Group- & Patient-level parameters"]` of patients who fall below this thresholds which is significantly more than expected by our model. Moreover, only `r bottom5percent["Full Model"]` of patients fall below our model's bottom 5% expectation based on prediction of true score with added measurement error (@tbl-contr, third column).^[As the data set we are inspecting contains only `r english(prev['reich2022','n'])` patients, small percentage predictions might easily be misaligned between model predictions and observed sample. However, the general observations that the sample contains significantly more outliers with respect to true score predictions but not for raw score predictions hold in our case even for larger intervals such as 80% ETIs and the conclusion of this section thus seem valid.] Assuming our model is a reasonable approximation of DRS-2 post-surgery change, we would not be surprised to see such a variability in the bottom range of such a data set, however, we should attribute most of it to measurement error rather than true score change variability. This would provide researchers with an information they can act upon by increasing sample size to offset lost efficiency due to measurement error or adopt analysis procedure that accounts for measurement error explicitly [@gelman2021]. Alternatively, researcher can hypothesize that the discrepancy between model predicted true score variability and observed values is not due to measurement error but rather reflects differences between populations. This line of reasoning could motivate researcher to account for population differences via procedures such as post-stratification [@deffner2022]. Finally, researcher can surmise that our model is inappropriate for description of part or the whole inspected data set. If this was the conclusion researchers made, they can easily use our model to generate predictions for their sample using model full posteriors we share in a plain text format on article's repository ([https://github.com/josefmana/dbs_cogPRED](https://github.com/josefmana/dbs_cogPRED)) and falsify it.**

## Predictive pre-surgery cognitive profile

**Instead of aiming to identify any pre-surgery cognitive variables that can be potentially predictive of post-surgery cognitive decline, we aimed to identify only these variables that are the most likely to be truly predictive. To achieve this goal, we applied the Bayesian Lasso and factor analysis to decrease false positive error rates of our analysis and validated effectiveness of these procedures via a set of simulations.**
...
## The role of affected STN sites
