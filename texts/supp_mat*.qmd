...
reff <-
  ...
  lapply(
    ...
    c("m0_linear","m1_lasso_doms","m2_lasso_tests"),
    function(i)
  ...
  mutate(
        ...
        `Model: ` =
            case_when(
                grepl("m0",model) ~ "time only",
                grepl("m1",model) ~ "factor scores",
                grepl("m2",model) ~ "test scores"
            ) %>% factor(
                levels = c("time only","factor scores","test scores"),
                ordered = T
            )
...
f2 <-
  ...
  labs( y = NULL ) +
  scale_fill_manual( values = alpha( cbPal[c(1,2,3)], .5 ) ) +
