...
# save them
for ( i in names(t2) ) write.table( x = t2[[i]], file = here( "tabs", paste0("pred_",i,"rob.csv") ), sep = ";", row.names = F, quote = F )
...
# save it
ggsave( plot = f2, file = here("figs","pred_vareffects.jpg"), dpi = 300, width = 6.6, height = 8 )
...
for ( i in names(f4) ) {
  ...
  ggsave(
    ...
    # specify output file
    file = here( "figs", paste0("pred_",i,"rob.jpg") ),
    dpi = 300,
...
# save & print it
gtsave( data = t2$test, filename = here("tabs","pred_testrob.docx") )
t2$test
...
# save & print it
gtsave( data = t2$factor, filename = here("tabs","pred_factorrob.docx") )
t2$factor
...
![Position of electrodes as estimated by Lead-DBS.](../figs/vat_electrodes.png){#fig-leads}
...
ggsave(
  plot = f8,
  filename = here("figs","vat_ttest.jpg"),
  dpi = 300,
  width = 7.7,
  height = 9.5
)
...
# save & print it
gtsave( data = t6, filename = here("tabs","vat_posteriors.docx") )
t6
