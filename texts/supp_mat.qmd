...
**To explore association between electrode localization and post-surgery cognitive decline we retrospectively gathered magnetic resonance imaging (MRI) data of patients included in our data set and estimated volume of activated issue (VAT) by stimulation parameters at time of MRI assessment and its intersection with subthalamic nucleus (STN) and its motor, associative and limbic sections. Only patients with all (i) pre-surgery MRI for STN localization, (ii) post-surgery MRI for electrode localisation, and (iii) stimulation parameters at time of MRI assessment for VAT computation were included. Pre-processing was conducted in Lead-DBS software ([lead-dbs.org](https://www.lead-dbs.org)) and is more closely describned in the main text. The outcome of this pre-processing was a set of estimates of the overlap between VAT and STN (and its components) for each included patient.**
...
$$P(DRS_i = DRS_{max}) = 1 - T(\vartheta,\mu_i,\sigma), for DRS_i \in N_{max}, N_{max} = \{i: drs_i = drs_{max}\}$$ $$DRS_i \sim~ t(\vartheta,\mu_i,\sigma), for DRS_i \in N_1, N_1 = \{i: drs_i < drs_{max}\}$$ $$\mu_i = \alpha + \delta_{time}time_i + \beta_{inclusion}inclusion_i + \delta_{inclusion}time_iinclusion_i + \bar{\alpha}_{id[i]} + \bar{\delta}_{id[i]}time_i$$
